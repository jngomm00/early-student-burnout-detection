...
def crear_dataset_enriquecido_60d(ruta_base_datos, ruta_salida):
    print("Iniciando generación del dataset enriquecido (Ventana: 60 días)...")
    ...
    # 3. Procesamiento de VLE (Interacciones) - Ventana de 60 días
    print("Procesando interacciones (VLE) a 60 días...")
    vle_60 = vle_df[vle_df['date'] <= 60].copy()

    # Crear identificador de semana (semana 0 a semana 8 aprox. para 60 días)
    ...
    # Cálculos derivados de inactividad a 60 días (Semanas 0 a 8 = 9 semanas)
    TOTAL_SEMANAS_60D = 9
    ...
    # 4. Procesamiento de Evaluaciones (Assessments) - Ventana de 60 días
    print("Procesando evaluaciones y retrasos a 60 días...")
    ...
    columnas_a_rellenar = [
                              ...
                              'semanas_actividad_plena', 'max_clicks_1_dia', 'entregas_realizadas_60d',
                              'nota_media_60d', 'retraso_medio_dias', 'total_entregas_tardias'
    ...
    # Añadimos el sufijo _60d para evitar sobrescribir los datasets
    archivo_train = os.path.join(ruta_salida, 'dataset_train_2013_enriquecido_60d.csv')
    archivo_test = os.path.join(ruta_salida, 'dataset_test_2014_enriquecido_60d.csv')

    df_train.to_csv(archivo_train, index=False)
    df_test.to_csv(archivo_test, index=False)

    print("-" * 50)
    print(f"Dataset de Entrenamiento (2013 - 60d) generado: {len(df_train)} filas.")
    print(f"Dataset de Prueba (2014 - 60d) generado: {len(df_test)} filas.")
...
if __name__ == "__main__":
    ...
    crear_dataset_enriquecido_60d(RUTA_ORIGEN, RUTA_DESTINO)