...
def crear_dataset_enriquecido_completo_30d(ruta_base_datos, ruta_salida):
    print("Iniciando generación del dataset enriquecido a 30 días (con variables sociodemográficas)...")
    ...
    print("Procesando interacciones (VLE) para los primeros 30 días...")
    ...
    TOTAL_SEMANAS_30D = 5 # Los 30 días abarcan las semanas 0, 1, 2, 3 y 4
    ...
    archivo_train = os.path.join(ruta_salida, 'dataset_train_2013_full_30d.csv')
    archivo_test = os.path.join(ruta_salida, 'dataset_test_2014_full_30d.csv')

    df_train.to_csv(archivo_train, index=False)
    df_test.to_csv(archivo_test, index=False)

    print("-" * 50)
    print(f"Dataset de Entrenamiento a 30 días (2013) generado: {len(df_train)} filas.")
    print(f"Dataset de Prueba a 30 días (2014) generado: {len(df_test)} filas.")
...
if __name__ == "__main__":
    ...
    crear_dataset_enriquecido_completo_30d(RUTA_ORIGEN, RUTA_DESTINO)